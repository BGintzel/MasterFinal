...
import utils
from euler_circle_generator.euler_circle_gen_duo import create_dataset, create_valid_dataset

dirname = os.path.dirname(__file__)
data_path = os.path.join(dirname, 'euler_circle_generator/generated_diagram')
data_path_test = os.path.join(dirname, 'euler_circle_generator/generated_diagram_test')
...
def get_dataloaders_reasoning_test(path=os.path.dirname(__file__), max_num_images=100000, batch_size=32):
    data, labels = [], []
    path = os.path.join(path, 'datasets')

    if not os.path.exists(data_path_test):
        create_valid_dataset()
        data, labels = load_data.load(data_path_test, max_num_images)

        utils.save_file(os.path.join(path, 'test_dataset'), data)
        utils.save_file(os.path.join(path, 'test_labels'), labels)
        print('datasets created')
    else:
        data = utils.load_file(os.path.join(path, 'test_dataset'))
        labels = utils.load_file(os.path.join(path, 'test_dataset'))
...
def get_new_dataloaders(data, labels, batch_size=32):
    labels = labels.astype(float)
    length = len(data)
