	...
	#print(d,center,self.center)
        if d > (radius + self.radius - tol):
            ...
            if radius > self.radius - tol:
                ...
                raise VennRegionException("Invalid configuration of circular regions (holes are not supported) negative d.")
        elif radius > self.radius+np.abs(d) - tol:
                # We are completely covered by that circle or we are the same circle
            return [VennEmptyRegion(), self]
        else:
		...
		#print('Contained case')
                a_1 = vector_angle_in_degrees(np.array([0,self.radius]))
                a_2 = vector_angle_in_degrees(np.array([0,-self.radius]))
                b_1 = vector_angle_in_degrees(np.array([0,radius]))
                b_2 = vector_angle_in_degrees(np.array([0,-radius]))
                ...
                i_arc1 = Arc(center, radius, a_1, a_2, True)
                i_arc2 = Arc(center, radius, b_2, b_1, True)
                intersection = VennArcgonRegion([i_arc1, i_arc2])
                return [self, intersection]
                #raise VennRegionException("Invalid configuration of circular regions (holes are not supported) None Intersection.")
