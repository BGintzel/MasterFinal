...
from euler_circle_generator.matplotlib_venn._math import find_distance_by_area, normalize_by_center_of_mass, tol
...
def solve_venn2_circles(venn_areas):
    ...
    if A_ab > tol:
        ...
        if A_b==A_ab:
            coords[1][0]+=random.uniform(radii[1]-radii[0],0)
            #print('B is smaller and contained',A_b,coords[1][0])
        elif A_a==A_ab:
            coords[1][0]+=random.uniform(radii[0]-radii[1],0)
            #print('A is smaller and contained',A_a,coords[1][0])
    else:
        # Zero intersection
        coords = np.zeros((2, 2))
        coords[1][0] = radii[0] + radii[1] + random.expovariate(lambd=5)  # The max here is needed for the case r_a = r_b = 0
        print('random coordinate generated',coords[1][0])
...
def venn2(subsets, set_labels=('A', 'B'), set_colors=('r', 'g'), alpha=0.4, normalize_to=1.0, ax=None):
    ...
    if set_labels is not None:
        ...
        labels = None
    return VennDiagram(patches, centers, radii)
